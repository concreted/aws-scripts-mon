...

{
  ...
  $parse_result = GetOptions(
    ...
    'custom-namespace:s' => \$custom_namespace,
    );
...
if ($custom_namespace) {
  if (length($custom_namespace) == 0) {
    exit_with_error("Must supply a >0 length string with --custom-namespace");
  }
  $input_ref->{'Namespace'} = $custom_namespace;
}
